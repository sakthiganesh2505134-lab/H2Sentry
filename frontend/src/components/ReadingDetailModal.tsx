...
import { X, ShieldAlert, CheckCircle2, AlertTriangle, Gauge } from 'lucide-react';
...
export const ReadingDetailModal: React.FC<ReadingDetailModalProps> = ({
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      case 'SAFE':
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold font-mono bg-emerald-950/80 text-emerald-400 border border-emerald-500/40">
            <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> SAFE ({reading.estimated_dose.toFixed(0)} ppm·min)
          </span>
        );
      case 'MODERATE':
      case 'ELEVATED':
      case 'WARNING':
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold font-mono bg-amber-950/80 text-amber-400 border border-amber-500/40">
            <AlertTriangle className="w-3.5 h-3.5 mr-1" /> ELEVATED ({reading.estimated_dose.toFixed(0)} ppm·min)
          </span>
        );
      case 'HIGH':
      case 'CRITICAL':
      case 'DANGER':
        return (
          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold font-mono bg-red-950/80 text-red-400 border border-red-500/40 animate-pulse">
            <ShieldAlert className="w-3.5 h-3.5 mr-1" /> DANGER ({reading.estimated_dose.toFixed(0)} ppm·min)
          </span>
        );
      default:
        return null;
      ...
      <div className="figma-card max-w-2xl w-full max-h-[90vh] overflow-y-auto border-figma-border">
        ...
        <div className="flex items-center justify-between p-5 border-b border-figma-border/70">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-xl bg-figma-card border border-figma-border flex items-center justify-center text-figma-accent">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="p-5 rounded-xl bg-figma-card border border-figma-border/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <span className="text-xs font-mono text-figma-textMuted uppercase tracking-wider">Cumulative Dose</span>
              <div className="flex items-baseline space-x-2 mt-1">
                <span className="text-3xl font-extrabold text-white tracking-tight font-mono">
                  {reading.estimated_dose.toFixed(0)}
                </span>
                <span className="text-sm font-semibold text-figma-textMuted">{reading.dose_unit}</span>
              </div>
              <div className="flex items-center space-x-2 mt-2">
                <span className="text-xs text-figma-textSecondary">
                  Equivalent 8-hour TWA: <strong className="text-white font-mono">{reading.equivalent_8h_twa_ppm.toFixed(1)} ppm</strong>
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
            <div className="p-3 rounded-lg bg-figma-card border border-figma-border/50">
              ...
              <span className="font-bold text-white block truncate">{reading.worker_name || 'James Rodriguez'}</span>
            ...
            <div className="p-3 rounded-lg bg-figma-card border border-figma-border/50">
              ...
              <span className="font-bold text-figma-accent block truncate">{reading.badge_id || 'MRPL-H2S-8821'}</span>
