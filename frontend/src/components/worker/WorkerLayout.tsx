...
export const WorkerLayout: React.FC<WorkerLayoutProps> = ({
  ...
  // Filter readings strictly for current worker
  const workerReadings = readings.filter((r) => !currentWorker || r.worker_id === currentWorker.id);
  const latestReading = workerReadings.length > 0 ? workerReadings[0] : null;
  ...
  return (
    <div className="w-full max-w-md mx-auto min-h-screen bg-figma-bg flex flex-col justify-between text-white relative select-none">
      ...
      {workerTab !== 'scan' && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-figma-surface/95 backdrop-blur-lg border-t border-figma-border/80 px-6 py-2.5 z-40">
          <div className="flex items-center justify-between">
            {/* Home Tab */}
            <button
              onClick={() => setWorkerTab('home')}
              className={`flex flex-col items-center space-y-1 transition-colors ${
                workerTab === 'home' ? 'text-figma-accent font-bold' : 'text-figma-textMuted hover:text-white'
              }`}
            >
            ...
            <div className="relative -top-5">
              <button
                ...
                className="w-14 h-14 rounded-full bg-figma-accent text-black flex items-center justify-center shadow-xl shadow-figma-accent/40 transform active:scale-95 transition-all ring-4 ring-figma-bg"
              >
            ...
            <button
              ...
              className={`flex flex-col items-center space-y-1 transition-colors ${
                workerTab === 'history' ? 'text-figma-accent font-bold' : 'text-figma-textMuted hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex flex-col items-center space-y-1 transition-colors ${
                workerTab === 'profile' ? 'text-figma-accent font-bold' : 'text-figma-textMuted hover:text-white'
              }`}
            >
